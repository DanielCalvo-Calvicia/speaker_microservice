...
class FakeSpeakerService(SpeakerPlaybackPort):
    ...
    async def play(self, request: PlayStreamInboundDTO) -> PlaybackOutboundDTO:
        setup_response = PlaybackOutboundDTO(success=self.success, message=self.message)
        request.acknowledge_setup(setup_response)
        if not self.success:
        ...
        try:
            ...
            return PlaybackOutboundDTO(success=False, message=str(e))
        return setup_response
...
def build_app(service: FakeSpeakerService) -> FastAPI:
    app = FastAPI()
    app.include_router(SpeakerHandler(service).router)
    return app
...
def test_stream_starts_with_stream_started():
    ...
    assert events[0]["payload"] == {"message": "ok", "sample_rate": 24000, "channels": 1}
    for event in events:
...
def test_inbound_adapter_exposes_only_required_business_routes():
    ...
    assert route_paths == {"/health", "/ready", "/available", "/process/stream/set"}
...
def test_ndjson_stream_started_only_is_accepted_without_audio():
    ...
    response = post_ndjson(client, ndjson_body(ndjson_event("stream_started", 1, {"sample_rate": 24000, "channels": 1})))
...
def test_ndjson_partial_bytes_base64_is_decoded_to_audio_bytes():
    ...
    response = post_ndjson(
        client,
        ndjson_body(
            ndjson_event("stream_started", 1, {"sample_rate": 24000, "channels": 1}),
...
def test_ndjson_partial_is_consumed_before_request_body_closes():
    async def run_probe() -> None:
        class StreamingProbeService(FakeSpeakerService):
            ...
            async def play(self, request: PlayStreamInboundDTO) -> PlaybackOutboundDTO:
                setup_response = PlaybackOutboundDTO(success=True, message="probe setup ok")
                request.acknowledge_setup(setup_response)
                try:
                    async for chunk in request.audio_stream:
                        if chunk:
                    ...
                    return PlaybackOutboundDTO(success=False, message=str(e))
                return PlaybackOutboundDTO(success=True, message="probe playback ok")
        ...
        async def body():
            yield ndjson_body(ndjson_event("stream_started", 1, {"sample_rate": 24000, "channels": 1}))
            yield ndjson_body(
                ndjson_event(
                    "partial", 2, {"bytes_base64": base64.b64encode(audio).decode("ascii")}
                )
            )
...
def test_ndjson_completed_does_not_end_request_or_play_output_bytes():
    ...
    response = post_ndjson(
        ...
        ndjson_body(
            ndjson_event("stream_started", 1, {"sample_rate": 24000, "channels": 1}),
            ndjson_event(
                "partial", 2, {"bytes_base64": base64.b64encode(first_audio).decode("ascii")}
            ),
            ndjson_event(
                "completed",
                3,
            ...
            ndjson_event(
                "partial", 4, {"bytes_base64": base64.b64encode(second_audio).decode("ascii")}
            ),
...
def test_ndjson_rejects_bad_sequence_with_error_event():
    ...
    response = post_ndjson(client, ndjson_body(ndjson_event("stream_started", 2, {"sample_rate": 24000, "channels": 1})))
...
def test_ndjson_rejects_audio_events_before_stream_started():
    ...
    response = post_ndjson(
        client,
        ndjson_body(
            ndjson_event("partial", 1, {"bytes_base64": base64.b64encode(b"audio").decode("ascii")})
        ),
    )

    events = parse_events(response)
    assert response.status_code == 200
    assert "stream_started must be the first event" in find_error_message(events)
...
def test_ndjson_rejects_malformed_json_with_error_event():
    ...
    events = parse_events(response)
    assert response.status_code == 200
    assert "event is not valid JSON" in find_error_message(events)
...
def test_ndjson_rejects_partial_missing_bytes_base64_with_error_event():
    ...
    events = parse_events(response)
    assert response.status_code == 200
    assert "missing required field 'bytes_base64'" in find_error_message(events)
...
def test_a_stream_announcing_another_format_is_rejected_with_an_error_event():
    client = build_client(FakeSpeakerService())

    response = post_ndjson(
        client,
        ndjson_body(ndjson_event("stream_started", 1, {"sample_rate": 44100, "channels": 2})),
    )

    message = find_error_message(parse_events(response))
    assert "44100 Hz x 2" in message and "24000 Hz x 1" in message


def test_non_positive_format_is_a_422_and_a_device_that_cannot_open_is_a_503():
    client = build_client(FakeSpeakerService())
    assert client.post("/process/stream/set?sample_rate=0", content=b"x").status_code == 422

    broken = build_client(FakeSpeakerService(success=False, message="hardware unavailable"))
    assert broken.post("/process/stream/set", content=b"x").status_code == 503


def test_available_probes_the_output_device_not_the_playback_state():
    class NoDevice(FakeSpeakerService):
        async def check_readiness(self):
            return ReadinessOutboundDTO(is_ready=False, message="Selected device has no output channels.")

    ok = build_client(FakeSpeakerService()).get("/available").json()["data"]
    down = build_client(NoDevice()).get("/available").json()["data"]

    assert ok == {"is_available": True, "reason": None}
    assert down == {"is_available": False, "reason": "Selected device has no output channels."}
